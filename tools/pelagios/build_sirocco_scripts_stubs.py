#!/usr/bin/env python3
"""Generate stub scripts.inc for every Sirocco map so map.json script references
resolve at link time. Each stub defines:
  ...
  one minimal placeholder per unique EventScript label referenced by the map.json
    (objects, coord triggers, bg signs).

Placeholders are intentionally minimal (lockall/releaseall/end for talk labels,
bare end for engine-invoked coord triggers). The pelagios-script-writer replaces
these with real content. Shared/empty labels (0x0) are NOT redefined.
...
def collect_labels(m):
    ...
    for b in m.get('bg_events', []):
        add(b.get('script'), 'talk')   # signs use msgbox; lockall/releaseall stub is safe
    return labels
...
for d in sorted(os.listdir(os.path.join(ROOT, 'data/maps'))):
    if not d.startswith('Sirocco_') and d != 'Sirocco_DustmouthPort':
        if not d.startswith('Sirocco'):
            continue
    ...
    for lab, kind in labels:
        lines.append(f'@ TODO(pelagios-script-writer): implement {lab}')
        lines.append(f'{lab}::')
        if kind == 'trigger':
            lines.append('\tend')
        else:
