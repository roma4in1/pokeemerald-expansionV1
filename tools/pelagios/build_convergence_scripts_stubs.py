#!/usr/bin/env python3
"""Generate stub scripts.inc for every Convergence map so map.json script
references resolve at link time. Mirrors build_ashenveil_scripts_stubs.py.
...
NO Tennyson / boat exception on Convergence (Storm Compass is the return; there is
no boarding object on this island). Every label is a plain TODO stub.
...
for d in sorted(os.listdir(os.path.join(ROOT, 'data/maps'))):
    if not d.startswith('Convergence_'):
        continue
