#!/usr/bin/env python3
"""Generate stub scripts.inc for every Gildhaven map so map.json script references
resolve at link time. Mirrors build_thalvern_scripts_stubs.py.
...
wiring (not a stub) - it sets VAR_TEMP_1 = PELAGIOS_ISLAND_GILDHAVEN and gotos the
...
TENNYSON_LABEL = 'GildhavenGoldportHarbor_EventScript_Tennyson'
TENNYSON_BODY = (
    f'@ Boat-boarding wiring (map-builder owns this; NOT dialogue).\n'
    f'{TENNYSON_LABEL}::\n'
    '\tlockall\n'
    '\tsetvar VAR_TEMP_1, PELAGIOS_ISLAND_GILDHAVEN\n'
...
for d in sorted(os.listdir(os.path.join(ROOT, 'data/maps'))):
    if not d.startswith('Gildhaven_'):
        continue
