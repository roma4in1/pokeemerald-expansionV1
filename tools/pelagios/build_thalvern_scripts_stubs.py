#!/usr/bin/env python3
"""Generate stub scripts.inc for every Thalvern map so map.json script references
resolve at link time. Mirrors build_schism_scripts_stubs.py.
...
EXCEPTION: the port's *_EventScript_Tennyson is written as REAL boat-boarding
wiring (not a stub) - it sets VAR_TEMP_1 and gotos the shared boat menu. This is
boat-system wiring, not dialogue, so it is map-builder scope (mirrors Schism).
...
# Tennyson boarding wiring (real, not a stub): sets the moored-island marker then
# enters the shared destination menu.
TENNYSON_LABEL = 'ThalvernTidespirePort_EventScript_Tennyson'
TENNYSON_BODY = (
    f'@ Boat-boarding wiring (map-builder owns this; NOT dialogue).\n'
    f'{TENNYSON_LABEL}::\n'
    '\tlockall\n'
    '\tsetvar VAR_TEMP_1, PELAGIOS_ISLAND_THALVERN\n'
